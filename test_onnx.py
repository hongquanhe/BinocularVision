...
class YOLOV5_ONNX(object):
    ...
    def infer(self,img_path):
        ...
        # c = c + 1
        # cv2.imwrite('img/output/' + str(num) + '.jpg', src_img)
        if det is not None and len(det):
            self.draw(src_img, det)
            # cv2.imwrite('img/output' + str(c) + '.jpg', src_img)
            print('OK')
        else:
            # cv2.imwrite('img/output'+str(c) + '.jpg', src_img)
            print('None')
    ...
    def draw(self,img, boxinfo):
        ...
        for *xyxy, conf, cls in boxinfo:
            ...
            # print('xyxy: ', xyxy)
            self.plot_one_box(xyxy, img, label=label, line_thickness=1)
        cv2.putText(img, "Defeat Number:", (10, 25), cv2.FONT_HERSHEY_COMPLEX, 0.5, [101, 67, 254],
                    thickness=1)
        cv2.putText(img, str(len(boxinfo)), (10, 50), cv2.FONT_HERSHEY_COMPLEX, 1.0, [0, 0, 0],
                    thickness=3)
            # self.plot_one_box(xyxy, img, label=label, color=colors[int(cls)], line_thickness=1)

        # cv2.namedWindow("dst",0)
        # cv2.imshow("dst", img)
        # cv2.imwrite('img/output'+str(c) + '.jpg', img)
        # cv2.waitKey(0)
...
if __name__=="__main__":
    num = 1
    model = YOLOV5_ONNX(onnx_path="models/best.onnx")

    # videoCapture = cv2.VideoCapture(videoPath)
    # for num in range(1, 301):
    #     model.infer(img_path='dataset/Pa_' + str(num) + '.bmp')
    model.infer(img_path='img/img_test/1.jpg')
