...
class WidgetCamera(QWidget):
    def __init__(self):
        ...
        self.yolo = YOLO5()
        # self.setStyleSheet('background-color:rgba(220,220,220,0.5)')
    ...
    def draw(self, qp):
        ...
        # 画框架背景
        qp.setBrush(QColor(220, 220, 220))  # 框架背景色
        qp.setPen(Qt.NoPen)
        ...
        if self.opened and self.image is not None:
            ...
            for obj in self.objects:
                ...
                # 画距离和坐标
                qp.drawText(ox, oy - 40, 'World Coordinate : ')
                qp.drawText(ox, oy - 20, '(' + str(round(obj['text_x'], 2)) + ', '
                            + str(round(obj['text_y'], 2)) + ', '
                            + str(round(obj['text_z'], 2)) + ')'+str(round(obj['text_dis'], 2)))
