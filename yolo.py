...
class YOLO5:
    ...
    def obj_detect(self, image):
        ...
        # if torch.from_numpy(img).ndimension() == 3:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    x = (xyxy[0] + xyxy[2]) / 2
                    y = (xyxy[1] + xyxy[3]) / 2
                    height_0, width_0 = image.shape[0:2]
                    # print(width_0)

                    if x < int(width_0 / 2):
                        iml = image[0:int(height_0), 0:int(width_0 / 2)]
                        imr = image[0:int(height_0), int(width_0 / 2):int(width_0)]
                        ...
                        # 可以使用上文的stereo_config.py给出的参数

                        dis = ((points_3d[int(y), int(x), 0] ** 2
                            + points_3d[int(y), int(x), 1] ** 2
                            + points_3d[int(y), int(x), 2] ** 2) ** 0.5) / 100
                        ...
                        text_x = (points_3d[int(y), int(x), 0] / 100)
                        text_y = (points_3d[int(y), int(x), 1] / 100)
                        text_z = (points_3d[int(y), int(x), 2] / 100)
                        text_dis = dis

                        objects.append({
                            'class': self.names[int(cls)], 'color': self.colors[int(cls)],
                            'confidence': conf.item(), 'x': xywh[0], 'y': xywh[1], 'w': xywh[2], 'h': xywh[3],
                            'text_x': text_x, 'text_y': text_y, 'text_z': text_z, 'text_dis': text_dis
                        })
                        # print(objects)
