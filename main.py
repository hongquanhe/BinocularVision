...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        if current < minimum:
            ...
            msg.setText(f'Current Python Version ({current}), Please updata to {minimum} Overall')
            msg.exec()
            sys.exit()

        self.setStyleSheet('color:rgb(220,220,220);'
                           'background-color:rgb(41,36,33);'
                           'font-size: 500%;'
                           'font-weight: bold;'
                           'font-family:consolas')
        self.setWindowTitle(f'{APP_NAME} {APP_VERSION}')
        self.setWindowIcon(QIcon('gui_img/yologo.png'))
    ...
    def load_yolo(self):
        """重新加载YOLO模型"""
        YOLOGGER.info(f'Update YOLO model: {self.settings.line_weights.text()}')
        # 目标检测
        ...
        if check:
            ...
            YOLOGGER.info('Model has been uploaded successfully.')
        else:
            # 配置有误，放弃加载模型
            YOLOGGER.warning('Incorrect configuration, give up loading model.')
    ...
    def update_status(self, text, ok=False):
        ...
        if ok:
            background += "rgba(209,186,116,0.5)"
        else:
            background += "rgba(160,238,225,0.5)"
    ...
    def update_info(self):
        ...
        while self.camera.detecting:
            ...
            # begin
            # self.info.add_current()
            # end
            # if len(self.camera.objects) > 0:
                # self.info.update_3D(self.camera.objects[0]['text_x'],
                #                     self.camera.objects[0]['text_y'],
                #                     self.camera.objects[0]['text_z'],
                #                     self.camera.objects[0]['text_dis'])
            time.sleep(0.2)
        # if len(self.camera.objects) > 0:
        #     self.info.update_3D(self.camera.objects[0]['text_x'],
        #                         self.camera.objects[0]['text_y'],
