"""
@Author:Fallen
@File:detect_double.py
@Time:2021/10/11 1:57

use python 3.6 'detector' as as default
"""
...
from stereo.dianyuntu import preprocess, undistortion, getRectifyTransform, draw_line, rectifyImage, \
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        if (x <= int(width_0 / 2)):
                            ...
                            print('Point (%d, %d)的%s距离左摄像头的相对距离为 %0.1f cm' % (x, y, label, dis))

                            # text_x = "x:%.1fmm" % (points_3d[int(y), int(x), 0] / 10)
                            # text_y = "y:%.1fmm" % (points_3d[int(y), int(x), 1] / 10)
                            # text_z = "z:%.1fmm" % (points_3d[int(y), int(x), 2] / 10)
                            text_x = "x:%.1fmm" % (points_3d[int(y), int(x), 0] / 100)
                            text_y = "y:%.1fmm" % (points_3d[int(y), int(x), 1] / 100)
                            text_z = "z:%.1fmm" % (points_3d[int(y), int(x), 2] / 100)
                            text_dis = "dis:%.1fmm" % dis

                            cv2.rectangle(im0, (int(xyxy[0] + (xyxy[2] - xyxy[0])), int(xyxy[1])),
                                          (int(xyxy[0] + (xyxy[2] - xyxy[0]) + 5 + 220), int(xyxy[1] + 150)), colors[int(cls)], -1);
                            cv2.putText(im0, text_x, (int(xyxy[0] + (xyxy[2] - xyxy[0]) + 5), int(xyxy[1] + 30)), cv2.FONT_ITALIC,
                                        1.2, (255, 255, 255), 3)
                            cv2.putText(im0, text_y, (int(xyxy[0] + (xyxy[2] - xyxy[0]) + 5), int(xyxy[1] + 65)), cv2.FONT_ITALIC,
                                        1.2, (255, 255, 255), 3)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='models/best_defect.pt', help='model.pt path(s)')
